...
import DocumentAnalysisWidget from "./components/DocumentAnalysisWidget";
import ImageGenerationWidget from "./components/ImageGenerationWidget";
...
function App() {
  ...
  useEffect(() => {
    ...
    // Document Analysis Listener
    const handleDocumentAnalysisUpdate = (data) => {
      console.log("Received document analysis update:", data);
      setDocumentData(data);
    };

    // Image Generation Listener
    const handleImageGenerationUpdate = (data) => {
      console.log("Received image generation update:", data);
      setImageData(data);
    };
    ...
    socket.current.on("document_analysis_update", handleDocumentAnalysisUpdate); // Listen for document analysis
    socket.current.on("image_generation_update", handleImageGenerationUpdate); // Listen for image generation
    ...
    return () => {
      ...
      if (socket.current) {
        ...
        socket.current.off("document_analysis_update", handleDocumentAnalysisUpdate);
        socket.current.off("image_generation_update", handleImageGenerationUpdate);
  ...
  const handleCloseDocumentAnalysisWidget = useCallback(() => {
    setDocumentData(null);
  }, []);

  const handleCloseImageGenerationWidget = useCallback(() => {
    setImageData(null);
  }, []);
    ...
    <div className="app-container">
      ...
      {documentData && (
        <DocumentAnalysisWidget
          ...
          onClose={handleCloseDocumentAnalysisWidget}
        />
      )}
      ...
      {imageData && (
        <ImageGenerationWidget
          imageData={imageData}
          onClose={handleCloseImageGenerationWidget}
        />
      )}
